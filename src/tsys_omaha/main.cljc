(ns tsys_omaha.main
  "Kotodama WASM entrypoint for the Tsys_omaha clean-room actor (L4) — Clojure port.
  ...
  facts (`tsys_omaha.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "tsys_omaha")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"tsys_omaha.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "tsys_omaha-compat" :tier tier :entities entities} 200])
...
(defn start! [] :tsys_omaha-compat/ready)
