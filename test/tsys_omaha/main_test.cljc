(ns tsys_omaha.main-test
  "Contract + behavioral test for the tsys_omaha-compat L4 actor (cljc port).
...
(deftest healthz
  (let [[body status] (m/healthz)]
    (is (= 200 status)) (is (= "tsys_omaha-compat" (:actor body))) (is (= (set m/entities) (set (:entities body))))))

#?(:clj (defn -main [& _]
          (let [{:keys [fail error]} (run-tests 'tsys_omaha.main-test)]
